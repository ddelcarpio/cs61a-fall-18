...
def replace_leaf(t, old, new):
    ...
    while is_leaf(t):
        if label(t) != old:
            ...
            return tree(new)
    replaced = [replace_leaf(b, old, new) for b in branches(t)]
    return tree(label(t),replaced)
...
def balanced(m):
    ...
    right_side = right(m)
    def torque(side):
        torque = total_weight(end(side)) * length(side)
        return torque

    if torque(left_side) == torque(right_side):
        balanced_left, balanced_right = True,True
        if is_mobile(end(left_side)):
            balanced_left = balanced(end(left_side))
        if is_mobile(end(right_side)):
            balanced_right = balanced(end(right_side))
        return balanced_left and balanced_right and True
    else:
        return False
def totals_tree(m):
    ...
    if not is_mobile(m):
        return tree(size(m))
    tree_totals = [totals_tree(end(left(m))), totals_tree(end(right(m)))]
    return tree(total_weight(m),tree_totals)
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,attempt):
        nonlocal balance
        nonlocal attempt_list
        if len(attempt_list) == 3:
            return  'Your account is locked. Attempts: ' + str(attempt_list)
        if attempt != password:
