...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        "*** YOUR CODE HERE ***"
        if self.exit: # Self = Place, we are checking for an exit
            exit.entrance = self # If we do have an exit, the exit's entrance is the Place (self)!
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if not insect.is_container and not self.ant.contained_ant:
                    self.ant.contain_ant(insect)
                elif insect.is_container and not self.ant.is_container:
...
class HarvesterAnt(Ant):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        colony.food += 1
        # Think of colony.food as a counter, not a list
        # END Problem 1
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        steps = 0  # The origin step is 0 and plus 1 at each step towards the next place
        min_range = self.min_range
        max_range = self.max_range

        def nearest_bee_helper(place, hive):
            nonlocal steps # Counter with nonlocal helps us keep track of the steps being moved.
            if place is hive or steps > max_range:
                return None # Conditions where bee cannot move.
            elif steps < min_range:  # Moving forward.
                steps += 1
                return nearest_bee_helper(place.entrance, hive)
            elif place.bees: # Random selector if >1 bee on a place.
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does nothing each turn, a WallAnt is useful because it has a large armor value"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 9
        if self.armor <= 0:
            self.place.contained_ant = self.contained_ant
...
class ScubaThrower(ThrowerAnt):
    """Subclass of ThrowerAnt that is more costly and watersafe, but
    otherwise identical to its base class."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.Impostor:
            ...
            while self.place.exit:
                if self.place.exit.ant:
                    if self.place.exit.ant not in self.double_amts:
                        self.place.exit.ant.damage *= 2
                        self.double_amts.append(self.place.exit.ant)
                    if self.place.exit.ant.container == True and self.place.exit.ant.ant and self.place.exit.ant.ant not in self.double_amts:
                        self.place.exit.ant.ant.damage *= 2
                        self.double_amts.append(self.place.exit.ant.ant)
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor -= amount
        if self.armor <= 0 and self.Impostor == True:
            self.place.remove_insect(self)
        else:
            bees_win()
        # END Problem 13
