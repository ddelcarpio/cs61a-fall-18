...
def group_by_centroid(restaurants, centroids):
    ...
    """
    # BEGIN Question 4
    restaurant_cluster = [[find_closest(restaurant_location(x),centroids),x] for x in restaurants]
    return group_by_first(restaurant_cluster)
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lon = mean([restaurant_location(restaurant)[0] for restaurant in cluster])
    lat = mean([restaurant_location(restaurant)[1] for restaurant in cluster])
    return [lon,lat]
    # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    array = max([find_predictor(user,reviewed,i) for i in feature_fns], key = lambda x: x[1])
    return array[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        restname = restaurant_name(restaurant)
        if restaurant in reviewed:
            dictionary[restname] = user_rating(user, restname)
        else:
            dictionary[restname] = predictor(restaurant)
    return dictionary
